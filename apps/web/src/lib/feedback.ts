// Writing to the person who makes Harmony.
//
...
export const FEEDBACK_KINDS: Record<FeedbackKind, KindCopy> = {
  bug: {
    ...
    placeholder:
      'I tended a habit and the petal for that area did not move...',
  },
  idea: {
    label: 'An idea',
    subject: 'Feature idea',
    prompt:
      'What would you like Harmony to do? It does not have to be a finished idea. A fair amount of what is here now started as somebody asking for it.',
...
export function composeFeedback(
  ...
  return {
    subject: `Harmony ${ctx.version} · ${FEEDBACK_KINDS[kind].subject}`,
    body: `${message.trim()}\n\n---\n${footer.join('\n')}\n`,
  };
}
