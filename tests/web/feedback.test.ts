...
// The composing and validating half of "Make Harmony Yours". Deliberately pure,
// so this suite needs no Supabase client, no Dexie, and no DOM.
...
describe('composeFeedback', () => {
  it('puts the sender\'s own words first, above the details', () => {
    const { body } = composeFeedback('bug', 'The petal did not move.', ctx);
    expect(body.startsWith('The petal did not move.')).toBe(true);
    expect(body).toContain('\n---\n');
  });

  it('attaches the version, device, and time so nobody has to type them', () => {
    const { body } = composeFeedback('bug', 'Something broke.', ctx);
    expect(body).toContain('Harmony 1.4.0');
  ...
  it('tells the two kinds apart in the subject line', () => {
    expect(composeFeedback('bug', 'x y z', ctx).subject).toBe('Harmony 1.4.0 · Bug report');
    expect(composeFeedback('idea', 'x y z', ctx).subject).toBe('Harmony 1.4.0 · Feature idea');
  });
...
describe('feedbackError', () => {
  it('accepts a message that says enough to act on', () => {
    expect(feedbackError('The bloom is stuck')).toBeNull();
  });
