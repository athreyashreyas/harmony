/**
 * The relay behind Me -> Make Harmony Yours.
 *
 ...
 *     back to the person who registered with Harmony, with no address to copy
 *     out by hand and no admin screen to build.
 *
 ...
 */
const FROM = Deno.env.get('FEEDBACK_FROM') ?? 'Harmony <onboarding@resend.dev>';
...
Deno.serve(async (req: Request) => {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      reply_to: replyTo,
      subject: subject || `Harmony · ${kind === 'idea' ? 'Feature idea' : 'Bug report'}`,
      text: body,
    }),
  });
