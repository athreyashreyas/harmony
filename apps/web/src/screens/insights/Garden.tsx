...
export default function Garden({ areas, habits, logs }: { areas: Area[]; habits: Habit[]; logs: Log[] }) {
  ...
  if (areas.filter((a) => a.archivedAt == null).length === 0) {
    return <p className="mt-8 text-sm text-ink-faint">Your garden grows as you tend. Add an area to begin.</p>;
  }

  return (
    <div className="mt-6">
      <p className="text-sm text-ink-muted">Every week, pressed and kept. Watch your life bloom and rest across the seasons.</p>
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        {weeks.map((w) => (
          <button
            ...
            className="flex flex-col items-center rounded-card bg-parchment-surface p-3 shadow-card transition-transform active:scale-[0.98]"
            style={openStart === w.start ? { boxShadow: '0 0 0 2px var(--accent-base)' } : undefined}
          >
            <MiniBloom petals={w.petals} />
            <span className="mt-1.5 text-xs font-medium text-ink-strong">{w.label}</span>
            {w.label !== w.dateLabel && <span className="text-[10px] text-ink-faint">{w.dateLabel}</span>}
            <span className="text-[11px] text-ink-faint">{caption(w.avg, w.isCurrent)}</span>
          </button>
        ))}
      </div>
...
function WeekDetail({ week }: { week: WeekBloom }) {
  ...
  return (
    <div className="pb-4">
      ...
      <p className="text-center font-serif text-lg text-ink-strong">{caption(week.avg, week.isCurrent)}</p>

      {week.petals.length > 0 && (
        <div className="mt-5 space-y-2.5 border-t border-parchment-raised pt-4">
          {tended.map((p) => (
            <div key={p.id} className="flex items-center gap-2.5">
              <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: p.color }} />
              <span className="min-w-0 flex-1 truncate text-sm text-ink-body">{p.name}</span>
              <span className="shrink-0 text-xs font-medium text-ink-muted">{Math.round(p.value * 100)}%</span>
            </div>
          ))}
          {resting.map((p) => (
            <div key={p.id} className="flex items-center gap-2.5">
              <span className="h-2.5 w-2.5 shrink-0 rounded-full opacity-30" style={{ backgroundColor: p.color }} />
              <span className="min-w-0 flex-1 truncate text-sm text-ink-faint">{p.name}</span>
              <span className="shrink-0 text-xs text-ink-faint">resting</span>
            </div>
          ))}
        </div>
      )}
