...
// Explicit, user-initiated refresh (pull to refresh): pull the latest state
// from the source database and also ask the service worker to check for a new
// app version. Returns when both have settled.
export async function manualRefresh(userId: string | null): Promise<void> {
  ...
  try {
    ...
    // A failed update check should never make the refresh feel broken.
  }
}
