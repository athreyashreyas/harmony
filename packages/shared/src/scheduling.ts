...
export function isHabitDueOn(habit: Habit, dateISO: string): boolean {
  ...
  switch (cadence.kind) {
    ...
    case 'times-per-week':
      ...
      return true;
    case 'every-n-days':
      return daysBetween(habit.startDate, dateISO) % Math.max(1, cadence.n) === 0;
    case 'every-n-weeks': {
      const days = daysBetween(habit.startDate, dateISO);
      return days % (7 * Math.max(1, cadence.n)) === 0;
    }
    case 'every-n-months': {
      const monthsApart =
        (date.getFullYear() - start.getFullYear()) * 12 + (date.getMonth() - start.getMonth());
      if (monthsApart < 0 || monthsApart % Math.max(1, cadence.n) !== 0) return false;
...
export function expectedCompletionsInWindow(cadence: Cadence, windowDays: number): number {
  ...
  switch (cadence.kind) {
    ...
    case 'every-n-weeks':
      return windowDays / (7 * Math.max(1, cadence.n));
    case 'every-n-months':
      return windowDays / (30 * Math.max(1, cadence.n));
  }
}
